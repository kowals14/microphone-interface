...
//  HAL_GPIO_WritePin(GPIOA, EN_Pin, GPIO_PIN_RESET);   // pull the EN pin low
//
...
//		  value[i] = HAL_ADC_GetValue(&hadc1);  // store adc value in value
//		  HAL_ADC_Stop(&hadc1);
//	  }
//	  HAL_GPIO_WritePin(selectpinsPort, selectPins[0], GPIO_PIN_SET);


	  //sprintf(message, "%d-%d-%d-%d-%d-%d-%d-%d\n\r", value[0], value[1], value[2], value[3], value[4], value[5], value[6], value[7]);
	  if(printgo == 1){HAL_UART_Transmit(&huart2, message, sizeof(message), 100);}
	  HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_15);
